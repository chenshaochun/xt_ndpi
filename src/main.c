                      ...
                      const struct iphdr *iph, uint16_t ipsize, const struct tcphdr *tcph)
{
	...
	curflow->detected_protocol = ndpi_detection_process_packet(ndpi_struct, curflow->ndpi_flow,
				(uint8_t *)iph, ipsize, time, src->ndpi_id, dst->ndpi_id);
	spin_unlock_bh (&ipq_lock);
...
static int __init ndpi_mt_init(void)
{
	...
	*/
	...
	for (i = 0; i <= NDPI_LAST_IMPLEMENTED_PROTOCOL; i++){
		...
		if ((i > 118 && i < 127) || (i > 139 && i < 146) || (i > 175 && i <= 218 ) || i == 70 || i == 133)
			nfndpi_protocols_http[i]=1;
		else
			nfndpi_protocols_http[i]=0;
	}
